...
struct header_t
{
    ...
    uint32_t    Array1Size;             // Total length in bytes of all "cubbyholes" in array #1 (always number of "cubbyholes" in array #1, multiplied by 2)
    uint32_t    Array1Count;            // Number of "cubbyholes" in array #1

    uint32_t    Array2Offs;             // File offset where array #2 starts
    uint32_t    Array2Size;             // Total length in bytes of all "cubbyholes" in array #1 (always number of "cubbyholes" in array #2, multiplied by 2)
    uint32_t    Array2Count;            // Number of "cubbyholes" in array #2

    uint32_t    Array3Offs;             // File offset where array #3 starts
    uint32_t    Array3Size;             // Total length in bytes of all "cubbyholes" in array #1 (always number of "cubbyholes" in array #3, multiplied by 2)
    uint32_t    Array3Count;            // Number of "cubbyholes" in array #3

    uint32_t    SampleHeaderOffs;       // File offset where sample headers start
    uint32_t    SampleHeaderSize;       // Total length in bytes of all sample headers (always number of sample headers multiplied by 16)
...
struct instrument_header_v2_t
{
    ...
    {
        uint16_t Index;                 // The number of another instrument header.
...
// The patch headers (which refers to the INTERNAL patches used in the waveset, as opposed to MIDI patches, which are EXTERNAL) assign one "cubbyhole" in array #1
// to each patch. The patch headers also affect certain properties of the patches, such as the pitch and amplitude envelopes. The number of patch headers is set
...
struct sample_header_t
{
    uint8_t NoteMax;                    // If a MIDI note is received with a MIDI note number above this value, the next sample header is used instead.
                                        // If the MIDI note number is still greater than NoteMax of the next sample header,
                                        // the sample header after the next sample header is used and so on until the MIDI note number is less than or equal NoteMax.
...
public:
    ...
    std::vector<instrument_header_t> InstrumentHeaders;
    std::vector<patch_header_t> PatchHeaders;

    std::vector<uint16_t> Array1;
    std::vector<uint16_t> Array2;
